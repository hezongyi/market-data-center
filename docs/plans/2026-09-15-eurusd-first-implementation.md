...
| P4 自主管理收口 | GBPUSD 复用、独立暂停/查询、归档与删除、用户指南；通过后按需增加已有周期 | E08；开发 agent 完成 E01–E09 用户旅程整体验收。无已确认需求则不扩大周期 | 待开始 |
| P5 版本交付 | 候选验证、版本/tag/发布回执、生产身份与单 writer 核对 | E10，生产范围按明确授权执行；不依赖 legacy parity/crypto 扩面 | 待开始 |

P0 首个 PR 应优先解决“可以放心操作的预览”，并让既有 UI 可访问；P1 不等待所有后端改完才展示。每阶段一个或数个小 PR，都引用验收编号。每次记录 commit、预览 URL、agent 验收范围、证据及结论，用户反馈如有则记录；不用百分比冒充可用性。
...
开发流程工具优化在独立工作分支实施：CI 改为 PR/main/manual、纯文档 PR 分流、安装与检查分离、局部检查入口、带代码身份与耗时的证据、预览访问卡。保留 hosted verify、代码改动完整矩阵与正式版本验证；使用方法以开发指南为准，合入 main 前不宣称远端流程已切换。
...
功能收口用一张 E/PV/DS 验收表加开发 agent 的执行与证据记录，不重做已经合入的小 PR，也不等待用户反馈。发布与生产激活另行记录，不因 spec approved 自动推进到生产。
