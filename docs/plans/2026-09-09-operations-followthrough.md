...
仓库已配置 `origin`，`.github/workflows/ci.yml` 与 `scripts/ci.sh` 使用同一入口；`scripts/ci.sh` 当前先执行 backend `ruff check src tests` 和 secret-scan lint，再执行测试、secret scan、Web UI build、隔离 service acceptance。本地真实验收对应代码 commit `ace645a`；GitHub Actions `Checks` run `34420773480`（run 16，job `verify`）已在 commit `2f3c743` 上成功，且包含共享 `scripts/ci.sh` 步骤。
