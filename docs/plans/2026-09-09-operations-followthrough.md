...
仓库已配置 `origin`，`.github/workflows/ci.yml` 与 `scripts/ci.sh` 使用同一入口；`scripts/ci.sh` 当前先执行 backend `ruff check src tests` 和 secret-scan lint，再执行测试、secret scan、Web UI build、隔离 service acceptance。本地真实验收对应代码 commit `ace645a`；GitHub Actions `Checks` run `34418651769`（run 14，job `verify`）已在 commit `9873389` 上成功，且包含共享 `scripts/ci.sh` 步骤。
...
- 最新本机 CI（commit `ace645a`）为 53 tests passed、backend lint、secret scan、Web UI build 和 service acceptance passed；systemd API/worker 与 smoke/provider-acceptance/monitor timers 保持 active+enabled，readiness、metrics、manifest endpoint 均已真实读取。启用 API key 后最新三 provider acceptance receipt 位于 `/home/quant/market_lake/evidence/data-center/acceptance-auth-pass-20260909T172131Z/`，Binance、yfinance、FRED 均 pass 并包含 manifest、hash、quality 和 API readback。GitHub Actions `Checks` run `34418651769`（run 14，job `verify`）在 commit `9873389` 上成功；economic/ingest/quality/maintenance consumer 继续保持 `not_migrated`。
...
- 追加鉴权验证（代码 commit `ace645a`）：systemd API 使用 `.env.local` 中的 0600 secret，未授权 ingest/retry 均返回 401，授权 fixture run 由 worker 完成并通过 quality；request ID 在 HTTP header、envelope、ledger 和 receipt 中关联。证据为 `/home/quant/market_lake/evidence/data-center/auth-boundary-20260909.json` 与 `/home/quant/market_lake/evidence/data-center/acceptance-auth-pass-20260909T172131Z/`。
