...
> 阶段编号变更说明：2026-09-14 之前本计划使用 P1–P5。规范第三次修订后改为 S0–S5，原因有二：一是把**基座**（迁移、WAL、批量原子提交、归属列、部署身份）从原 P1 中拆出，作为不改变任何外部行为、可独立合并的前置阶段；二是把**影子调度**（S2，只记录决策不派发）插在真实派发之前，用真实 coverage 先取得决策正确性的证据。原 P1 的其余内容成为 S1，原 P2–P3 拆为 S2/S3/S4，原 P4–P5 合并为 S5。下表给出对应关系。
...
- `retention-audit` 与 provider acceptance 的解耦（spec 9.3）是**先行独立修复**：它不属于本计划的交付批次，但必须在 S5 接管前以独立小发布完成并留 receipt；不得并入接管批次，也不得因为“接管时会一起处理”而推迟。

## 2. 阶段与验收映射

| 阶段 | 原编号 | 主要代码落点 | 实施工作 | 对应验收 |
| --- | --- | --- | --- | --- |
| **S0 基座** | 原 P1 的 schema 部分 + 新增项 | `runs/ledger.py`、`deployment.py`、`operations_views.py`、`scripts/*`、`backend/tests/` | 顺序迁移与 schema 版本；WAL 与有界 busy 处理；批量幂等原子入队；注入时钟；`jobs` 归属列（计划/execution/step）与到期索引；`runs` 可索引列 + SQL 侧读模型；deployment 逻辑哈希表清单与 `service_names`；receipt action 登记；多进程争用与备份恢复回归 | AC05 基础、AC22、AC24 的基础部分 |
| **S1 计划注册表** | 原 P1 其余 | 新增 production task module、`maintenance_tasks.py`、`api/app.py`、`catalog/snapshot.py`、`webui/src/lib/api.ts`、`webui/src/pages/MaintenancePage.tsx` | 计划/配置版本/所有权表与唯一约束；`preview/create/read/change`（含 pause/resume/archive/delete）；alias 兼容；`/production/*` 路由与 `MUTATING_ROUTES` 分类；`/capabilities` 增补；计划列表与详情 UI；目录矩阵（已计划/未计划/不可用/漂移）与只读清单 | AC01、AC07、AC16、AC19、AC20、AC23 |
| **S2 时间与影子调度** | 原 P2 的一部分 | 新增 `scheduler_main.py`、任务模块、`deploy/systemd/market-data-center-scheduler.service`、`api/app.py` | 计划时间模型（manual/once/fixed_rate/fixed_delay/daily，注入时钟）；`reconcile()` 决策路径；lease + fencing；`/operations/scheduler`；tick receipt；以 `dispatch_enabled=false` 记录“此刻会派发哪些计划/窗口/步骤” | AC02、AC03、AC04、AC14 |
| **S3 真实派发与执行** | 原 P2、P3 的派发部分 | `ingest/worker.py`、`runs/ledger.py`、任务模块 | execution/step/progress 持久化；按归属列与暂停状态反连接领取；run_now/retry 与 `/runs/{id}/retry` 归属联动；coalesce、backlog、预算与公平轮转；provider 退避；单 worker 派发上限（默认 1，可配置） | AC06、AC08、AC13、AC15 的部分 |
| **S4 依赖闭环** | 原 P3 的依赖部分 | `platform.py`、`maintenance_runner.py`、`derived_maintenance_runner.py`、`transform.py`、`ingest/process.py`、`catalog/snapshot.py` | 提取复用 planners；动态窗口与 backlog；固定输入 part 引用与跨进程重建；recipe 拓扑依赖与环检测；publication 游标与 reconciliation；受影响范围重算；旧 CLI 接同一任务模块 | AC09–AC12、AC15、AC18 的跨 provider 部分 |
| **S5 接管与默认启用** | 原 P4、P5 | `deploy/systemd/`、`deployment.py`、`scripts/browser_acceptance.cjs`、runbook、release checklist | 浏览器端到端闭环与移动端；旧入口盘点与暂停导入；阻新、收口、核对 ledger 后启用 canary；观察、回滚演练；文档与证据收口 | AC15、AC17、AC18、AC21、AC24 |
...
7. 把新增调度表加入 deployment 逻辑哈希的表清单，把 scheduler 服务加入 `service_names`，把调度 receipt 的 action 登记到运维 receipt 视图。
8. 备份/恢复回归必须覆盖新增表与输入引用；确认恢复流程不覆盖既有目标的行为不变。

## 4. S1 计划注册表检查点

1. 建计划、配置版本、所有权三类持久化结构；所有权键在 `enabled/paused` 计划间唯一，冲突返回稳定错误码，归档与删除在同一事务释放。
2. 接 `preview`、`create`、`read`、`change`；`change` 覆盖 pause/resume/run_now/archive/delete 与 `expected_version` 乐观锁；编辑不改变 `desired_state`。
...
5. 配置版本持久化解析时的配置摘要，并实现 registry 漂移检测（`health=config_drift`、停止派发、要求确认）。
...
7. 确认 `retention-audit` 已作为独立单元运行并留有 receipt；确认派生入口已改用 release venv 与 deployment manifest，不再引用仓库 checkout。
8. 更新 runbook、service 安装说明、版本兼容资料与发布证据；只有 spec 全部 AC 有证据后才关闭功能交付。
...
| 基座与迁移 | 从当前生产库升级、迁移重跑、WAL 下争用压测、备份/恢复含新表与输入引用、deployment 身份覆盖调度表 |
...
| 统一 gate | `bash scripts/ci.sh all`，Python 3.10/3.11/3.12、Node 22、Ruff、兼容/依赖/secret/operations/browser/service 验证 |
...
- 2026-09-14：规范完成第二、三次修订（缺口 G1–G14、四项已确认决策、AC19–AC24）。本计划随之对齐：P1–P5 改为 S0–S5 并给出对应关系，新增 S0 基座检查点、S1 注册表检查点、S4 依赖闭环检查点，把 `retention-audit` 解耦列为接管前必须完成的独立工作项，并在 S5 接管步骤中补入派生入口改用 release venv 的确认项。
