...
日期：2026-09-10
状态：implementation complete；本地与真实环境验收通过，等待 protected-main hosted CI 与 `v0.2.0` release closure
...
- R5：package/Web UI semantic version 已提升为 `0.2.0`，release note 和自动化 release receipt 生成器已提交。最终 closure 仍需合并后 protected-main `verify` 全绿、自动创建 `v0.1.0` GitHub release、推送 immutable `v0.2.0` tag，并从该 tag 执行 post-release smoke/rollback rehearsal。
