...
日期：2026-09-15
状态：approved；P0 基础范围及 P1 PV06 implemented / user accepted
来源：维护者确认的 EURUSD 优先闭环与阶段验收方案。P0 已交付 PV01、PV02、PV04、PV05、PV07；P1 已交付 PV06，身份与有效派发状态收口于 2026-09-15 经维护者复验通过；PV03、PV08 的完整范围留待 P2/P3。

2026-09-16 验收委托修订：维护者授权开发 agent 自行完成预览、阶段及集成验收，通过后继续已授权流程，不等待用户确认。执行与证据要求见[开发指南 §2](../development-guide.md#2-交付循环)；历史用户验收事实保留。

## 1. 目标与环境

Draft PR 尚未完成时即提供运行中的完整 WebUI 地址和访问方法，供用户随时体验。开发 agent 负责实际操作验收，用户反馈为可选，不阻塞通过后的继续工作。分支预览、main 集成验收、生产是不同环境；不能依靠生产发布来展示开发中的界面。
...
管理入口为 `bash scripts/dev-preview.sh start|status|stop|card --id <preview-id>`。持久预览位于仓库 ignored `.preview/<id>/`，也可显式指定受控 preview base；用户操作数据保留，stop 不删除目录。默认目录仍随整个 worktree 删除，需跨 worktree 保留时必须使用外置 base 或预先备份。
...
- card：只读生成可直接交付的访问卡，区分运行代码与工作区身份；可附 `--ssh-target` 转发命令及 `--receipt` 检查记录，不输出凭据。停止或身份不匹配返回非零；不把健康或单项检查结果自动声明为产品验收通过。
- stop：仅停止已核实属于本 preview-id 的进程组，校验 PID 与启动身份；不能按公共端口或模糊进程名杀进程。保留数据与日志；显式数据清理为独立操作。
- 切换版本：同一预览不静默改变代码或重置数据；先显示新旧身份和必要重启。预览保留到阶段交接，agent 验收通过不自动关闭用户体验入口；结束时告知停止与数据保留位置。
...
| PV07 | 预览可用后及时向用户提供地址、实际访问方法和 3–5 步操作卡；开发 agent 记录验收证据与结论，通过后继续，不等待用户确认；预览保留到阶段交接，无未声明自动重置 |
| PV08 | live 显式选择后仍只写沙箱，范围/预算可核对；模拟证据与真实源证据分开 |

P0 先验收隔离与既有界面可用；P1 已补齐 Router、页面身份与有效派发状态相关 PV06 验收。完整 raw→derived 场景随 P2/P3 补齐 PV03，live 沙箱随 P2 补齐 PV08；不得因此提前将所有 PV 标为通过。实现 PR 同步在开发指南填写已经验证的命令、访问方法和限制。
