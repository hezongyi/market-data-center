...
- 最终 immutable deployment 为 `65131589a832-f5de3553`，source commit `65131589a832b824df206aa1cede87c345ff3b2d`。activation receipt `aac6625bde7e46c09ae343b17ef37b27` 为 `pass`，canonical 与 ledger hash 均保持不变；最终 readiness 为 `ready`，read/write available，operational snapshot fresh。
- 正式 parity receipt `f306eacc3cd94332aed78c002e9719cc` 为 `pass`（最终代码增强后的 receipt `ad40f44762854d25856e567486b205b6` 之前版本仅作中间证据）。在 `[2026-08-28,2026-09-11)` 上共读取 12 行 BID；首/中/末三个固定窗口分别为 3/4/5 行，每个使用 `page_size=2` 跨越多个 cursor page，单一 pagination snapshot、0 duplicates，并保存稳定 hash。
- 最终 parity 的 60 秒观察期为 7 个 Data Center metrics samples，`failed_delta=0`、failure rate `0.0`，consumer repeated reads failure rate `0.0`；capacity 始终为 `warning`，before/after free ratio 均为 `0.1168406990808636`。unsupported selector 的 Data Center 与 consumer flag-on/off 错误语义均已执行并记录，network policy 以 sanitized `direct`/timeout `30s` 记录，不泄露 URL 或凭据。
- feature-flag cutover receipt `d1e257b59d474953bccf74532950c8ff` 和 rollback receipt `a00624ee39e943a8bf9acd99e82d3d95` 分别为 `pass`，均绑定 Data Center deployment 与 consumer commit；未迁移 inventory 明确记录 68,331,636 条、5,853,971,477 bytes raw-only 数据且 `bulk_migration_allowed=false`。
