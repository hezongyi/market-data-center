...
7. **部署身份与证据**：新增调度状态表必须进入 deployment 的逻辑哈希表清单，且该哈希必须按 `rowid` 流式计算、在激活路径中成本有界，并把每表行数与耗时记入 stage/activate receipt；不得用排除无界表的方式回避成本，也不得为此删除账本历史；scheduler 服务必须加入 deployment 的 `service_names`，使 activation/rollback 会重启并校验它；调度 receipt 的 action 必须登记到运维 receipt 视图的 action 列表，否则在运维页面不可见。
...
| AC22 基座与迁移 | WAL 下多进程争用无 `database is locked` 失败；到期扫描有界且走索引；批量入队在中断与响应丢失下不产生重复 run；迁移可从现有生产库升级且可重跑；备份/恢复保留计划、输入引用与进度；deployment 逻辑哈希覆盖调度表，且其激活路径成本有界并在 receipt 中记录每表行数与耗时 |
