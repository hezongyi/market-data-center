...
function App() {
  ...
  const loadBars = () => fetch(`/api/v1/bars?provider=${encodeURIComponent(provider)}&symbol=${encodeURIComponent(symbol)}&timeframe=1d`).then(r => r.json()).then(r => setBars(r.data));
  return <main><header><h1>Market Data Center</h1><span className="status">{health}</span></header>
    ...
    <section><h2>Data Explorer</h2><input value={provider} onChange={e => setProvider(e.target.value)} aria-label="Provider" /><input value={symbol} onChange={e => setSymbol(e.target.value)} aria-label="Symbol" /><button onClick={loadBars}>Load bars</button>{bars.length > 0 && <table><tbody>{bars.map((bar, i) => <tr key={i}><td>{String(bar.bar_ts)}</td><td>{bar.open}</td><td>{bar.high}</td><td>{bar.low}</td><td>{bar.close}</td></tr>)}</tbody></table>}</section>
  </main>;
}
