...
export function usePreferences() { const value = useContext(Context); const additions: Record<string, string> = { "Password changed. Sign in again.": "密码已修改，请重新登录", "Initialized. Sign in.": "初始化完成，请登录", "Set the first password to enable login.": "请先设置初始密码以启用登录", "Initialize password": "初始化密码", "Checking authentication status…": "正在检查认证状态…", "Signed in as": "当前登录用户", "Current password": "当前密码", "New password": "新密码", "Change password": "修改密码", Login: "登录", Logout: "退出", Password: "密码", Username: "用户名" }; return { ...value, t: (text: string) => value.locale === "zh-CN" ? dictionary[text] ?? additions[text] ?? text : text }; }
