...
def run_fred_ingest(*, series_id: str, root: Path, connector: FredConnector | None = None, start: str | None = None, end: str | None = None, ledger=None, run_id: str | None = None) -> dict:
    ...
    payload = {"run_id": run_id, "dataset_id": "economic_observations", "schema_version": ECONOMIC_OBSERVATIONS_SCHEMA_VERSION, "series_id": series_id, "provider": "fred", "connector_version": getattr(resolved_connector, "version", "1"), "input_hash": input_hash, "status": "pass", "row_count": len(rows), "min_date": min(r["observation_date"] for r in rows), "max_date": max(r["observation_date"] for r in rows), "path": str(path), "output_hash": output_hash, "quality_summary": {"status": "pass", "finding_count": 0, "findings": []}, "created_at": datetime.now(timezone.utc).isoformat()}
    if ledger is not None:
