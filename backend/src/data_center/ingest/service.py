...
def run_fixture_ingest(job: IngestJob, root: Path, ledger=None, run_id: str | None = None) -> dict:
    ...
    payload = {"run_id": run_id or str(uuid4()), "job_id": job.job_id, "status": "pass", "dataset_id": job.dataset_id, "row_count": len(rows), "path": str(path), "output_hash": output_hash, "created_at": datetime.now(timezone.utc).isoformat()}
