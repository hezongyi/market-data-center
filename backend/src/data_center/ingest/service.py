...
def run_fixture_ingest(job: IngestJob, root: Path, ledger=None, run_id: str | None = None) -> dict:
    ...
    payload = {"run_id": resolved_run_id, "job_id": job.job_id, "status": "pass", "dataset_id": job.dataset_id, "schema_version": SCHEMA_VERSION, "provider": job.provider, "connector_version": getattr(connector, "version", "1"), "input_hash": input_hash, "row_count": len(rows), "min_ts": min(row.bar_ts for row in rows).isoformat(), "max_ts": max(row.bar_ts for row in rows).isoformat(), "paths": [str(path) for path in paths], "output_hash": output_hash, "quality_summary": {"status": "pass", "finding_count": 0, "findings": []}, "created_at": datetime.now(timezone.utc).isoformat()}
