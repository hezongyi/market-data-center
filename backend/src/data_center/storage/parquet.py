...
from data_center.catalog.paths import market_bars_path, provider_bars_path
...
def write_market_bars(root: Path, rows: Iterable[MarketBar], *, part_id: str | None = None) -> list[Path]:
    records = list(rows)
    if not records:
        raise ValueError("cannot write empty market_bars dataset")
    import polars as pl
    ...
    for year in sorted({record.bar_ts.year for record in records}):
        year_records = [record.model_dump() for record in records if record.bar_ts.year == year]
        first = year_records[0]
        target = market_bars_path(root, provider=first["provider"], asset_class=first["asset_class"],
                                  symbol=first["symbol"], timeframe=first["timeframe"],
                                  price_basis=first["price_basis"], year=year)
        target.mkdir(parents=True, exist_ok=True)
        path = target / f"part-{shared_part_id}.parquet"
        if path.exists():
            raise FileExistsError(f"refusing to overwrite immutable part: {path}")
