...
def provider_bars_path(root: Path, *, provider: str, asset_class: str, symbol: str, timeframe: str, year: int,
                       managed_dataset_id: str | None = None) -> Path:
    target = root / "provider_bars"
    if managed_dataset_id:
        target /= f"managed_dataset={managed_dataset_id}"
    return target / f"provider={provider}" / f"asset_class={asset_class}" / f"symbol={symbol}" / f"timeframe={timeframe}" / f"year={year}"
