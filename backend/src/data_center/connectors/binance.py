...
class BinanceConnector:
    ...
    def fetch_bars(self, job: IngestJob) -> list[ProviderBar]:
        ...
        response = requests.get(self.endpoint, params={"symbol": job.symbol.upper(), "interval": interval, "startTime": int(job.start.timestamp() * 1000), "endTime": int(job.end.timestamp() * 1000), "limit": 1000}, timeout=20)
