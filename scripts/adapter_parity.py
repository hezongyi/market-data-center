...
def main():
    ...
    report = {"checked_at": datetime.now(timezone.utc).isoformat(), "status": "pass", "datasets": []}
    for provider, symbol in [("binance", "BTCUSDT"), ("yfinance", "SPY")]:
        ...
        report["datasets"].append({"provider": provider, "rows": len(local), "hash": hashlib.sha256(encode(local).encode()).hexdigest()})
